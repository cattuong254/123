...
function apiOpenAICompatibleEndpoints(app) {
  ...
  app.post(
    ...
    async (request, response) => {
      ...
      #swagger.requestBody = {
          description: 'Send a prompt to the workspace with full use of documents as if sending a chat in zexLLM. Only supports some values of OpenAI API. See example below.',
          required: true,
          content: {
            "application/json": {
              example: {
                ...
                {"role":"assistant", content: "zexLLM is...."},
  ...
  app.get(
    ...
    async (request, response) => {
      /*
      #swagger.tags = ['OpenAI Compatible Endpoints']
      #swagger.description = 'List all the vector database collections connected to zexLLM. These are essentially workspaces but return their unique vector db identifier - this is the same as the workspace slug.'
      #swagger.responses[200] = {
        content: {
          "application/json": {
            "schema": {
