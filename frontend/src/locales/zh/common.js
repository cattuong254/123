...
const TRANSLATIONS = {
  ...
  welcomeMessage: {
    part1:
      "欢迎使用 ZexAiLLM，这是由 Mintplex Labs 开发的开源 AI 工具，可以将任何东西转换为您可以查询和聊天的训练有素的聊天机器人。ZexAiLLM 是一款 BYOK（自带密钥）软件，因此除了您想使用的服务外，此软件不收取订阅费、费用或其他费用。",
    part2:
      "ZexAiLLM 是将强大的 AI 产品（如 OpenAi、GPT-4、LangChain、PineconeDB、ChromaDB 等）整合在一个整洁的包中而无需繁琐操作的最简单方法，可以将您的生产力提高 100 倍。",
    part3:
      "ZexAiLLM 可以完全在您的本地计算机上运行，几乎没有开销，您甚至不会注意到它的存在！无需 GPU。也可以进行云端和本地安装。\nAI 工具生态系统每天都在变得更强大。ZexAiLLM 使其易于使用。",
  ...
  api: {
    title: "API 密钥",
    description: "API 密钥允许持有者以编程方式访问和管理此 ZexAiLLM 实例。",
    link: "阅读 API 文档",
  ...
  llm: {
    title: "LLM 首选项",
    description:
      "这些是您首选的 LLM 聊天和嵌入提供商的凭据和设置。重要的是，这些密钥是最新的和正确的，否则 ZexAiLLM 将无法正常运行。",
    provider: "LLM 提供商",
  },
  ...
  transcription: {
    ...
    provider: "转录提供商",
    "warn-start":
      "在 RAM 或 CPU 有限的计算机上使用本地耳语模型可能会在处理媒体文件时停止 ZexAiLLM。",
  ...
  embedding: {
    ...
    "desc-end":
      "嵌入是将文本转换为矢量的过程。需要这些凭据才能将您的文件和提示转换为 ZexAiLLM 可以用来处理的格式。",
    provider: {
      title: "嵌入引擎提供商",
      description: "使用 ZexAiLLM 的本机嵌入引擎时不需要设置。",
    },
  },
  ...
  vector: {
    title: "向量数据库",
    description:
      "这些是 ZexAiLLM 实例如何运行的凭据和设置。重要的是，这些密钥是最新的和正确的。",
    provider: {
      title: "向量数据库提供商",
  ...
  multi: {
    ...
    password: {
      title: "密码保护",
      description:
        "用密码保护您的ZexAiLLM实例。如果您忘记了密码，那么没有恢复方法，所以请确保保存这个密码。",
    },
    instance: {
      title: "实例密码保护",
      description:
  ...
  privacy: {
    title: "隐私和数据处理",
    description:
      "这是您对如何处理连接的第三方提供商和ZexAiLLM的数据的配置。",
    llm: "LLM 选择",
    embedding: "嵌入首选项",
    vector: "向量数据库",
    anonymous: "启用匿名遥测",
